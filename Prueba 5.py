...
def crear_tablas():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS Movimientos (
            ...
            tipoMovimiento BIT NOT NULL, -- 0 = Abono, 1 = Cargar
...
class CuentaCorriente:
    ...
    def _registrar_movimiento(self, monto, tipo, descripcion):
        ...
        Args:
            ...
            tipo (int): 0 = Abono, 1 = Cargar.
            descripcion (str): Descripción del movimiento.
        """
        con = crear_conexion()
        query = f"""
            INSERT INTO Movimientos (idCtaCte, tipoMovimiento, Monto, descripcion)
            VALUES ({self.ID}, {tipo}, {monto}, '{descripcion}')
        """
        objCursor = con.cursor()
        objCursor.execute(query)
        con.commit()
    ...
    def exportar_csv_ctacte():
        """Exporta todos los registros de la tabla CtaCte a CSV."""
        con = crear_conexion()
        cursor = con.cursor()
    ...
    def exportar_csv_movimientos():
        """Exporta todos los registros de la tabla Movimientos a CSV."""
        con = crear_conexion()
        cursor = con.cursor()
...
cuenta3.abonar(20000, "Deposito sueldo")
cuenta1.cargar(5000, "Pago servicios")
cuenta2.abonar(30000, "Transferencia recibida")
