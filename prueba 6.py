...
class CuentaCorriente:
    ...
    def exportar_csv_movimientos():
        """Exporta todos los registros de Movimientos a CSV incluyendo descripción dinámica."""
        con = crear_conexion()
        cursor = con.cursor()
        ...
        # Columnas para CSV
        columnas = ["IdMovimientos", "idCtaCte", "tipoMovimiento", "Monto", "Descripcion"]
        ...
        with open('Movimientos.csv', 'w', newline='', encoding='utf-8') as archivo:
            ...
            for fila in resultados:
                tipo = fila[2]
                descripcion = "abono" if tipo == 0 else "carga"
                escritor.writerow(list(fila) + [descripcion])

        con.close()
        print("Exportación de Movimientos con descripción completada correctamente.\n")
